...
**Git提交**: `f61365d` 修复多个关键缺陷并增强功能

#### 核心Bug修复
- 修复分镜角色关联逻辑错误（scene.shots误用为角色ID列表）
- 修复场景提取时角色信息丢失（未保存characters字段）
- 修复首帧图片/视频提示词模板完全无效（硬编码覆盖配置）
- 修复character_ids格式兼容性问题（支持字符串和数组）
- 修复剧本片段提取失败导致分镜遗漏剧情问题

#### 新增功能
- 提示词统一管理页面（支持7个阶段的提示词编辑）
- 详细的LLM调试日志输出（输入/输出/解析结果）
- Scene模型增强character_names和script_segment字段
...
#### 详细日志输出
- 剧本解析阶段：显示LLM输入Prompt和原始响应
- 分镜设计阶段：显示场景信息、剧本片段、生成的分镜列表
- 提示词生成阶段：显示图片/视频提示词的生成过程
- 角色ID匹配：显示character_ids的原始值、类型和匹配结果

#### 数据模型增强
- Scene模型新增`character_names`字段：存储场景关联的角色名称列表
- Scene模型新增`script_segment`字段：存储场景对应的剧本片段
- 修复分镜角色关联：通过角色名匹配而非错误的ID匹配

#### 提示词模板修复
- 首帧图片提示词模板：现在正确使用配置而非硬编码
- 视频提示词模板：现在正确使用配置而非硬编码
- character_ids格式：支持字符串"char_001"和数组["char_001"]两种格式

### 📝 文档更新
- 更新ROADMAP.md：标记已完成任务，调整版本规划
- 更新CHANGELOG.md：添加最新的Bug修复和功能增强
